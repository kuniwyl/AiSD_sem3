...
public class ProductionSelectionSortTest {
    ...
    private static void addFileTitle(String test, int mode) {
        try {
            BufferedWriter writer = new BufferedWriter(
                    new FileWriter("src\\test\\java\\pl\\edu\\pw\\ee\\proSelection.txt", mode == 1 ? false : true));
            writer.append(test + "\n");
            writer.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void addResultToFile(int test, long end, long start) {
        try {
            BufferedWriter writer = new BufferedWriter(
                    new FileWriter("src\\test\\java\\pl\\edu\\pw\\ee\\proSelection.txt", true));
