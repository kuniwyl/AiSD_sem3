...
public class ProductionLinearProbingTest {
    ...
    public static void set() {
        try {
            BufferedWriter writer = new BufferedWriter(
                    new FileWriter("src\\test\\java\\pl\\edu\\pw\\ee\\resourse\\resultsLinear.txt", false));
            writer.append("");
            writer.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void getToFile(long[] input, long[] get, int title) {
        try {
            BufferedWriter writer = new BufferedWriter(
                    new FileWriter("src\\test\\java\\pl\\edu\\pw\\ee\\resourse\\resultsLinear.txt", true));
            writer.append(title + "\n");
            for (int i = 0; i < 30; i++) {
    ...
    public void test() {
        ...
        for (int x = 0; x < 10; x++) {
            ...
            for (int j = 0; j < 30; j++) {
                hashTable = new HashLinearProbing<>(sizes[x]);
